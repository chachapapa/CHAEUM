    ...
    <div
      className={
        isSearchBarOpened && location.pathname.includes('/profile')
          ? 'flex flex-col items-center justify-around sticky top-0 left-0 z-30 w-full min-h-[112px] bg-white transition-all'
          : 'flex flex-col items-center justify-center sticky top-0 left-0 z-30 w-full min-h-[56px] bg-white'
      }
    >
            ...
            <div className="w-full z-50 absolute top-[112px] flex flex-col flex-grow overflow-auto bg-white items-center">
              {searchUserList.map((user, index) => (
                <div key={index} className="w-11/12">
                  <FriendProfileCard user={user} index={index} type="search" />
                </div>
              ))}
            </div>
          ) : null}
